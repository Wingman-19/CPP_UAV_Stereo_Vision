...
#define CENTER_PER_THRESH 15 //Threshold for the percentage of pixels in the center that are above the disThresh
#define PER_THRESH 20	//Threshold for the percentage of pixels in a section that are above the disThresh
#define VELO 10	//This is velocity when the UAV must move into another section (Subject to change)
#define VELO_THETA 21.86	//This is the angle (in degrees) at which the UAV will move when moving to a corner section

//////////////////////////////////////////////////////////////////////////////////////////////////////
//
//	VELO_THETA was found by finding the dimensions (in pixels) of the center section: 628 x 252
//
//	Then THETA could be calculated by using the equation: arctan( 252 / 628)
//
//	Resulting in: 21.86 degrees
//
//////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		while (key != 'q') {
		    ...
		    if (zed.grab(runtime_parameters) == SUCCESS) {
				...
				cv::line(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols / 2 - 314, 0),
								cv::Point(depth_image_ocv_display.cols / 2 - 314, depth_image_ocv_display.rows),
								0,
								1);
				cv::line(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols / 2 + 314, 0),
								...
								1);
				cv::line(depth_image_ocv_display,
								cv::Point(0, depth_image_ocv_display.rows / 2 - 126),
								cv::Point(depth_image_ocv_display.cols, depth_image_ocv_display.rows / 2 - 126),
								0,
								1);
				cv::line(depth_image_ocv_display,
								cv::Point(0, depth_image_ocv_display.rows / 2 + 126),
...
void partitionCalc(sl::Mat& disparityMap, float *sectionValues)
{
	...
	int startWidth[] = { 0, disparityMap.getWidth() / 2 - 314, disparityMap.getWidth() / 2 + 314 };
	//An array of values that seperates the disparity image across its height
	int startHeight[] = { 0, disparityMap.getHeight() / 2 - 126, disparityMap.getHeight() / 2 + 126 };
...
int selectSection(const float *sectionValues)
{
	...
	int minPos = 4;
	//If the center is clear then return this section
	if(minPercent < CENTER_PER_THRESH)
		return minPos;
	else
	{
		minPercent = sectionValues[0];
		minPos = 0;
	}
	//Get the section number that has the lowest percentage
	for (int i = 0; i < 9; i++)
		if (sectionValues[i] < minPercent && i != 4)
		{
	...
	switch (section)
	{
		case 0:
			...
			cout << "The UAV is moving up and left.";
														//set_velocity(0, VELO * -1 * cos(VELO_THETA), VELO * -1 * sin(VELO_THETA), sp);
			break;
		case 1:
			//Increase altitude
		...
		case 2:
			...
			cout << "The UAV is moving up and right.";
														//set_velocity(0, VELO * cos(VELO_THETA), VELO * -1 * sin(VELO_THETA), sp);
			break;
		case 3:
			//Move left
		...
		case 6:
			...
			cout << "The UAV is moving down and left.";
														//set_velocity(0, VELO * -1 * cos(VELO_THETA), VELO * sin(VELO_THETA), sp);
			break;
		case 7:
			//Decrease altitude
		...
		case 8:
			...
			cout << "The UAV is moving down and right.";
														//set_velocity(0, VELO * cos(VELO_THETA), VELO * sin(VELO_THETA), sp);
			break;
		default:
