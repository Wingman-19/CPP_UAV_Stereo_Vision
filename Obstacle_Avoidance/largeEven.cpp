	...
	while (key != 'x')	//Used to exit the program
	{
		...
		while (key != 'q') {
		    ...
		    if (zed.grab(runtime_parameters) == SUCCESS) {
				...
				//Shows the rectangles that partition the image
				...
				cv::rectangle(depth_image_ocv_display,
								...
								cv::Point(depth_image_ocv_display.cols / 2 + 314, 252),
								29,
								1);
				cv::rectangle(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols - 628, 0),
								cv::Point(depth_image_ocv_display.cols, 252),
								58,
								1);

				cv::rectangle(depth_image_ocv_display,
								cv::Point(0, depth_image_ocv_display.rows / 2 - 126),
								cv::Point(628, depth_image_ocv_display.rows / 2 + 126),
								87,
								1);
				cv::rectangle(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols / 2 - 314, depth_image_ocv_display.rows / 2 - 126),
								cv::Point(depth_image_ocv_display.cols / 2 + 314, depth_image_ocv_display.rows / 2 + 126),
								116,
								1);
				cv::rectangle(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols - 628, depth_image_ocv_display.rows / 2 - 126),
								cv::Point(depth_image_ocv_display.cols, depth_image_ocv_display.rows / 2 + 126),
								145,
								1);

				cv::rectangle(depth_image_ocv_display,
								cv::Point(0, depth_image_ocv_display.rows - 252),
								cv::Point(628, depth_image_ocv_display.rows),
								174,
								1);
				cv::rectangle(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols / 2 - 314, depth_image_ocv_display.rows - 252),
								cv::Point(depth_image_ocv_display.cols / 2 + 314, depth_image_ocv_display.rows),
								203,
								1);
				cv::rectangle(depth_image_ocv_display,
								cv::Point(depth_image_ocv_display.cols - 628, depth_image_ocv_display.rows - 252),
								cv::Point(depth_image_ocv_display.cols, depth_image_ocv_display.rows),
								232,
								1);
...
void partitionCalc(sl::Mat& disparityMap, float *sectionValues)
{
	...
	int startWidth[] = { 0, disparityMap.getWidth() / 2 - 314, disparityMap.getWidth() - 628 };
	//An array of values that seperates the disparity image across its height
	int startHeight[] = { 0, disparityMap.getHeight() / 2 - 126, disparityMap.getHeight() - 252 };
	...
	for (int i = 0; i < 9; i++)
	{
		...
		int endW = startWidth[col] + 628;	//Used to know where the width of the current section ends
		int endH = startHeight[row] + 252;	//Used to know where the height of the current section ends
